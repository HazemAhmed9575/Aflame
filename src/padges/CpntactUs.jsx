...
import {
  ...
  setMessage,
  setSubject,
} from "../redex/slices/contactSlice";
function CpntactUs() {
  const { email, subject, message, loding, errEmail, errsubject, errmessage } =
  ...
  return (
    <div className=" w-full flex flex-col items-center gap-6 bg-[#212529] p-10">
      ...
      <form
        ...
        <div className="flex flex-col gap-2">
          <label
            className={`font-bold text-lg ${errsubject && "text-red-600"}`}>
            {" "}
            {errsubject ? "Invalid Your Subject" : "Your Subject"}
          </label>
          <input
            value={subject}
        ...
        <div className="flex flex-col gap-2">
          <label
            className={`font-bold text-lg ${errmessage && "text-red-600"}`}>
            {" "}
            {errmessage ? "Invalid Your massage" : "Your Message"}
          </label>
          <textarea
            value={message}
            ...
            name="massage"
            rows="4"
            className="bg-[#212529] p-3 text-[#0dcaf0] border-[#0dcaf0] border-2 rounded-md focus:outline-none focus:shadow-[0_0_0_0.25rem_rgba(13,110,253,0.25)]"></textarea>
        </div>

        <div className="flex justify-center  w-full">
          <button
            type="submit"
            onClick={() => dispatch(sendContact())}
            className={`py-1.5 px-10 text-base  text-[#0dcaf0] border-2 rounded-md border-[#0dcaf0] hover:bg-[#0dcaf0] hover:text-black ${
              loding && "pointer-events-none bg-gray-700"
            }`}>
            {loding ? <Spinner /> : "Sudmit"}
          </button>
        </div>
      </form>
    </div>
